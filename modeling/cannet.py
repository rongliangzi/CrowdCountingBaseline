...
class CANNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        if not load_weights:
            ...
            mod.load_state_dict(torch.load('/home/datamining/Models/vgg16-397923af.pth'))
            self._initialize_weights()
#            print("VGG",list(mod.state_dict().items())[0][1])#要的VGG�?
            fsd=collections.OrderedDict()
            for i in range(len(self.frontend.state_dict().items())):#10个卷�?（weight，bias�?20个参�?
...
#        print('w6',w6.mean())
        ...
        fi=(w1*s1+w2*s2+w3*s3+w6*s6)/(w1+w2+w3+w6+1e-9)
#        print('fi',fi.mean())
#        fi=fv
        x=torch.cat((fv,fi),1)
